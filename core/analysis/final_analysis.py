...
)  # Function to format the final analysis prompt.
from core.agents import (
    get_architect_for_phase,
)  # Import for dynamic model configuration
...
class FinalAnalysis:
    ...
    async def run(
        self, consolidated_report: Dict, project_structure: Optional[List[str]] = None
    ) -> Dict:
        """
        ...
        try:
            ...
            prompt = format_final_analysis_prompt(
                consolidated_report, project_structure or []
            )

            logger.info(
                "[bold]Final Analysis:[/bold] Creating Cursor rules from consolidated report"
            )
            ...
            logger.info(
                "[bold green]Final Analysis:[/bold green] Rules creation completed successfully"
            )
