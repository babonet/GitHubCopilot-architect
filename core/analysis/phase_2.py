...
from typing import (
    ...
    List,
    Optional,
)  # Used for type hinting, making code more readable
...
)  # Function to parse agent definitions
from core.agents import (
    get_architect_for_phase,
)  # Added import for dynamic model configuration
...
class Phase2Analysis:
    ...
    # ====================================================
    async def run(self, phase1_results: Dict, tree: Optional[List[str]] = None) -> Dict:
        """
        ...
        try:
            ...
            # ====================================================
            prompt = format_phase2_prompt(phase1_results, tree or [])

            logger.info(
                "[bold]Phase 2:[/bold] Creating analysis plan using configured model"
            )
            ...
            # ====================================================
            analysis_plan_response = await self.architect.create_analysis_plan(
                phase1_results, prompt
            )
            ...
            if "error" in analysis_plan_response and analysis_plan_response["error"]:
                logger.error(
                    f"[bold red]Error:[/bold red] {analysis_plan_response['error']}"
                )
            ...
            # ====================================================
            plan_text = analysis_plan_response.get(
                "plan", ""
            ...
            agents = parse_agents_from_phase2(
                plan_text
            )  # Parse the agent definitions from plan text

            if agents:
                logger.info(
                    f"[bold green]Success:[/bold green] Found {len(agents)} agents in the analysis plan"
                )
                for i, agent in enumerate(agents):
                    files_count = len(agent.get("file_assignments", []))
                    logger.info(
                        f"  [bold cyan]Agent {i+1}:[/bold cyan] {agent.get('name', 'Unknown')} with {files_count} files"
                    )
            # If no agents found, try the fallback approach directly
            else:
                logger.info(
                    "[bold yellow]Warning:[/bold yellow] No agents found from standard parsing, trying fallback"
                )
                try:
                    fallback_agents = extract_agent_fallback(plan_text)
                    if fallback_agents:
                        logger.info(
                            f"[bold green]Success:[/bold green] Fallback found {len(fallback_agents)} agents"
                        )
                        agents = fallback_agents
                    else:
                        logger.warning(
                            "[bold yellow]Warning:[/bold yellow] Fallback parsing couldn't find any agents"
                        )
                except Exception as e:
                    logger.error(
                        f"[bold red]Error:[/bold red] Fallback parsing failed: {str(e)}"
                    )
