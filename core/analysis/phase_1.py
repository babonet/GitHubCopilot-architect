...
)  # Function to get the appropriate architect for a phase
...
class Phase1Analysis:
    ...
    async def run(self, tree: List[str], package_info: Dict) -> Dict:
        ...
        context = {"tree_structure": tree, "package_info": package_info}
        ...
        architect_indices = {
            architect: i for i, architect in enumerate(self.architects)
        }
        ...
        async def run_architect_with_logging(architect, ctx):
            ...
            logging.info(
                f"[bold cyan]Agent {agent_num}:[/bold cyan] {architect.name} starting analysis"
            )
            result = await architect.analyze(ctx)
            logging.info(
                f"[bold green]Agent {agent_num}:[/bold green] {architect.name} completed analysis"
            )
            return result

        architect_tasks = [
            run_architect_with_logging(architect, context)
            for architect in self.architects
        ]
        results = await asyncio.gather(*architect_tasks)

        logging.info(
            "[bold green]Phase 1:[/bold green] All agents have completed their analysis"
        )

        # Return the results with phase information.
        return {"phase": "Initial Discovery", "findings": results}
