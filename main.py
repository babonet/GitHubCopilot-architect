...
from typing import (
    Dict,
    List,
)  # For type hinting (helps with code readability and error checking)
import logging  # For logging messages
from rich.console import Console  # For rich text output in the console
from rich.logging import RichHandler  # For enhanced logging
...
import asyncio  # Ensure asyncio is defined
...
if os.path.exists(".env"):
    ...
    logging.warning(
        "No .env file found. Make sure API keys are set in environment var."
    )
...
# Fix undefined names and adjust line lengths
...
class ProjectAnalyzer:
    ...
    async def run_final_analysis(
        self, consolidated_report: Dict, tree: List[str]
    ) -> Dict:
    ...
    async def analyze(self) -> str:
        ...
        with Progress(
            ...
            ),  # Show a description of the current task
            ...
            console.print(
                (
                    "[dim]Running three concurrent agents: Structure Agent, "
                    "Dependency Agent, and Tech Stack Agent...[/dim]"
                )
            )

            task1 = progress.add_task(
                "[green]Running analysis agents...", total=1
            )  # Add a task for Phase 1 with a total
            tree_with_delimiters = get_project_tree(
                self.directory
            )  # Generate the directory tree using the enhanced tree generator

            # Remove delimiters for analysis
            tree_for_analysis = tree_with_delimiters
            if (
                len(tree_with_delimiters) >= 2
                and tree_with_delimiters[0] == "<@tree_generator.py project_structure>"
            ...
            package_info = (
                {}
            )  # Placeholder for package information (you would parse package.json here)
            self.phase1_results = await self.run_phase1(
                tree_for_analysis, package_info
            )  # Run Phase 1
            ...
            console.print(
                "[green]✓[/green] Phase 1 complete: All three agents have finished their analysis"
            )
            ...
            self.phase2_results = await self.run_phase2(
                self.phase1_results, tree_for_analysis
            )
            ...
            if agent_count > 0:
                console.print(
                    f"[dim]Running {agent_count} specialized analysis agents on their assigned files...[/dim]"
                )
            else:
                console.print(
                    "[dim]Running specialized analysis on project files...[/dim]"
                )

            task3 = progress.add_task("[yellow]Analyzing files in depth...", total=1)
            self.phase3_results = await self.run_phase3(
                self.phase2_results, tree_for_analysis
            )
            ...
            console.print(
                "[yellow]✓[/yellow] Phase 3 complete: In-depth analysis finished"
            )

            # --- Phase 4: Synthesis ---
            console.print("\n[bold magenta]Phase 4: Synthesis[/bold magenta]")
            console.print(
                "[dim]Synthesizing findings from all previous analyses...[/dim]"
            )
            ...
            console.print(
                "[dim]Consolidating all results into a comprehensive report...[/dim]"
            )
            ...
            self.final_analysis = await self.run_final_analysis(
                self.consolidated_report, tree_for_analysis
            )  # Run the final analysis with project structure
            ...
            console.print(
                "[white]✓[/white] Final Analysis complete: Cursor rules created"
            )
        ...
        analysis.extend(
            [
                ...
                "-" * 30,
                json.dumps(
                    self.phase1_results, indent=2
                ),  # Include Phase 1 results (formatted as JSON)
                "\n",
                f"Phase 2: Methodical Planning (Config: {phase2_model})",
                "-" * 30,
                self.phase2_results.get(
                    "plan", "Error in planning phase"
                ),  # Include Phase 2 plan (or an error message)
                "\n",
                ...
                "-" * 30,
                self.phase4_results.get(
                    "analysis", "Error in synthesis phase"
                ),  # Include Phase 4 analysis
                "\n",
                f"Phase 5: Consolidation (Config: {phase5_model})",
                "-" * 30,
                self.consolidated_report.get(
                    "report", "Error in consolidation phase"
                ),  # Include Phase 5 report
                "\n",
                f"Final Analysis (Config: {final_model})",
                "-" * 30,
                self.final_analysis.get(
                    "analysis", "Error in final analysis phase"
...
@click.option(
    "--path", "-p", type=str, help="Path to the project directory"
)  # Define a command-line option for the project path
@click.option(
    "--output", "-o", type=str, help="Output file path (deprecated, no longer used)"
)  # Mark as deprecated
def main(path: str, output: str):
    ...
    try:
        ...
        )  # Print a message indicating which project is being analyzed
        analyzer = ProjectAnalyzer(directory)  # Create a `ProjectAnalyzer` instance
        start_time = time.time()  # Start timing here
        analysis_result = asyncio.run(
            analyzer.analyze()
        )  # Run the analysis (this is an asynchronous operation)
        ...
        phase1_results = (
            analyzer.phase1_results if hasattr(analyzer, "phase1_results") else {}
        )
        phase2_results = (
            analyzer.phase2_results if hasattr(analyzer, "phase2_results") else {}
        )
        phase3_results = (
            analyzer.phase3_results if hasattr(analyzer, "phase3_results") else {}
        )
        phase4_results = (
            analyzer.phase4_results if hasattr(analyzer, "phase4_results") else {}
        )
        consolidated_report = (
            analyzer.consolidated_report
            if hasattr(analyzer, "consolidated_report")
            else {}
        )
        final_analysis = (
            analyzer.final_analysis if hasattr(analyzer, "final_analysis") else {}
        )
        ...
        if success:
            console.print(
                f"[green]Cleaned cursor rules file: removed text before 'You are...'[/]"
            )
        else:
            if "not found" in message:
                ...
                console.print(
                    f"[yellow]Pattern 'You are' not found in cursor rules file[/]"
                )
            else:
                console.print(f"[yellow]{message}[/]")

        console.print(
            f"[green]Individual phase outputs saved to:[/] {directory}/phases_output/"
        )
        console.print(
            f"[green]Cursor rules created at:[/] {directory}/.cursorrules"
        )  # Inform about .cursorrules file
        console.print(
            f"[green]Cursor ignore created at:[/] {directory}/.cursorignore"
        )  # Inform about .cursorignore file
        console.print(
            f"[green]Execution metrics saved to:[/] {directory}/phases_output/metrics.md"
        )  # Inform about metrics file
